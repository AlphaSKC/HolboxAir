...
export default function MonthlyQuotesChart() {
    ...
    const fetchData = async () => {
        try {
            ...
            setApiData(response.cotizacionesMensuales);
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const processChartData = (data: any) => {
        ...
        return {
            ...
            yData: sortedData.map((item: { totalCotizaciones: number }) => item.totalCotizaciones),
        };
    };
    ...
    return (
        <Box sx={{ width: "100%", overflowX: "auto", bgcolor: "#ffff", borderRadius: 5, boxShadow: "0 0 10px #e8e8e8", padding: 2, mt: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
            <Typography variant="h5" sx={{ fontWeight: "bold", mb: 2 }}>Cotizaciones mensuales</Typography>
